...
CREATE TABLE cms.nodetags (
	...
	tag text NOT NULL,
	locale text,
	CONSTRAINT pk_nodetags PRIMARY KEY (node, tag, locale),
	CONSTRAINT fk_nodetags_nodes FOREIGN KEY (node)
		REFERENCES cms.nodes (node),
	CONSTRAINT ck_nodetags_tag CHECK (char_length(tag) <= 64),
	CONSTRAINT ck_nodetags_locale CHECK (locale IS NULL OR char_length(locale) <= 32)
);
...
CREATE TABLE cms.menus (
	...
	CONSTRAINT ck_menus_menu CHECK (char_length(menu) <= 64),
	CONSTRAINT ck_menus_description CHECK (description IS NULL OR char_length(description) <= 512)
);
...
CREATE TABLE cms.menuitems (
	...
	data jsonb NOT NULL,
	CONSTRAINT pk_menuitems PRIMARY KEY (item, menu),
	CONSTRAINT fk_menuitems_menus FOREIGN KEY (menu)
		REFERENCES cms.menus (menu) ON DELETE CASCADE,
	CONSTRAINT fk_menuitems_menuitems FOREIGN KEY (parent, menu)
		REFERENCES cms.menuitems (item, menu) ON DELETE CASCADE,
	CONSTRAINT ck_menuitems_item CHECK (char_length(item) <= 255),
	CONSTRAINT ck_menuitems_parent CHECK (parent IS NULL OR char_length(parent) <= 255)
);
