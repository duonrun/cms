-- Sample nodes for integration testing (SQLite version)
-- Assumes basic-types.sqlite.sql has been loaded
...
FROM cms_types WHERE handle = 'test-home';
...
SELECT
    ...
    1,
    1
FROM cms_nodes WHERE uid = 'test-homepage';
...
FROM cms_types WHERE handle = 'test-article';
...
FROM cms_types WHERE handle = 'test-block';
